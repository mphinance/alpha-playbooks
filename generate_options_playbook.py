...
VENUS_STORAGE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "backups")
...
def fetch_ticker_data(ticker):
    ...
    # Prepare the specific structure for the OPTIONS.DASHBOARD UI
    ...
    # 2. IV Stats
    iv_val = iv
    iv_rank = 25.0 # Placeholder
    iv_percentile = 30.0 # Placeholder
    ...
    dashboard_data = {
        ...
        "volumeStats": {
            "callVolume": 0, "putVolume": 0, "totalVolume": 0, "pcRatioVol": pc_ratio_vol,
            "callOpenInt": 0, "putOpenInt": 0, "totalOpenInt": 0, "pcRatioOi": pc_ratio_oi,
        ...
        "pcCharts": {
            ...
            "volRatios": [pc_ratio_vol] * 10,
            "oiRatios": [pc_ratio_oi] * 10
        }
    }
...
def save_json(data, ticker):
    ...
    # Also save as latest.json for the dynamic dashboard
...
def save_series_json(ticker, chart_data, ema_data):
    ...
    # Also save as latest_series.json
...
def deploy_to_vultr(ticker, html_path):
    ...
    try:
        ...
        # 4. Create symlink for latest.html
...
def main():
    ...
    if data:
        ...
        update_dashboard() # Update dynamic dashboard
        
        # New Deployment Steps
        deploy_to_vultr(args.ticker, html_path)
        
        # Deploy root index and docs/index.html to Vultr
        print("Deploying Global Indices...")
        subprocess.run(["scp", "index.html", f"{VULTR_ALIAS}:{VULTR_WEB_ROOT}/index.html"], check=True)
